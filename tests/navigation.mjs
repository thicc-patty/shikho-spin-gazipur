...
try{
 ...
 await page.getByRole('button',{name:'পরের ধাপ'}).click();await page.goBack();assert.equal(await page.getByLabel('তোমার নাম',{exact:true}).inputValue(),'আরিফ হাসান');await page.goForward();await page.getByRole('radio',{name:'নবম শ্রেণী',exact:true}).check();await page.locator('#consent').check();await page.getByRole('button',{name:'পরের ধাপ'}).click();await page.getByRole('radio',{name:'বিজ্ঞান',exact:true}).check();await page.getByRole('button',{name:'এবার চাকা ঘোরাই'}).click();
