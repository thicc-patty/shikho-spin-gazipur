...
export type EntryRow = {
  id: string; phone: string; name: string; study_group: string; class_level: string; event_id: string; event_info: EventInfo;
...
export function crmPayload(row: EntryRow, kind: string) {
  return { schemaVersion: 1, kind, lead: { phone: row.phone, name: row.name, studyGroup: row.study_group, classLevel: row.class_level },
